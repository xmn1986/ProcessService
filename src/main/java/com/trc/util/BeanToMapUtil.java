		...
		try {
			...
			for (int i = 0; i < propertyDescriptors.length; i++) {
				...
				if (map.containsKey(propName)) {
					...
					//将mongodb返回的Array类型转换成JSONArray
					if(StringUtils.equals(JSONArray.class.getName(), descriptor.getPropertyType().getName())){
						if(StringUtils.equals(ArrayList.class.getName(), propVal.getClass().getName())){
							propVal = JsonUtil.parseToJsonArray(propVal);
						}
					}
					//处理数字类型
