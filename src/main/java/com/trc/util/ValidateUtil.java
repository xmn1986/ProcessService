...
public class ValidateUtil {
	...
	* @throws
	 */
	public static void paramNullCheck2(HttpServletRequest request,  String... validParams){
		Map<String, Object> params = CommonUtil.getRequestParam(request);// 获取所有参数
		for (String key : validParams) {
			if(!StringUtils.contains(key, ":"))
				throw new ParamValidException("校验参数"+key+"不是\"参数名:参数说明\"格式");
	...
	* @throws
	 */
	public static void paramCheck(Object param,  String...paramKeys){
		JSONObject jsonObject = JsonUtil.parseToJson(param);
		for (String key : paramKeys) {
	...
	/**
	 * 检查参数中是否包含paramKeys中的参数
	 * @param param
	 * @param validParams 校验参数，格式：“参数名:参数说明”, 如多个参数校验 “name:姓名”,“age:年龄”,
	 */
	public static void paramCheck2(Object param,  String...validParams){
		JSONObject jsonObject = JsonUtil.parseToJson(param);
		for (String key : validParams) {
			if(!StringUtils.contains(key, ":"))
				throw new ParamValidException("校验参数"+key+"不是\"参数名:参数说明\"格式");
			String[] keyVal = key.split(":");
			if(!jsonObject.has(keyVal[0])){
				throw new ParamValidException("参数"+keyVal[1]+keyVal[0]+"必须传入");
			}
		}
	}
	...
	* @throws
	 */
	public static void paramNullCheck(Object param,  String...paramKeys){
		JSONObject jsonObject = JsonUtil.parseToJson(param);
		for (String key : paramKeys) {
			if(!jsonObject.has(key)){
				throw new ParamValidException("缺少参数"+key);
				...
				if(StringUtils.isEmpty(val))
					throw new ParamValidException("参数"+key+"的值不能为空");
			}
		}
	}
	...
	* @throws
	 */
	public static void paramNullCheck2(Object param,  String... validParams){
		JSONObject jsonObject = JsonUtil.parseToJson(param);
		for (String key : validParams) {
			if(!StringUtils.contains(key, ":"))
				throw new ParamValidException("校验参数"+key+"不是\"参数名:参数说明\"格式");
	...
	public static void basePageValid(HttpServletRequest request){
		ValidateUtil.paramNullCheck2(request, "pageIndex:当前页码", "start:开始记录数", "limit:每页记录数"); 
	}
